...
const HHCard = ({ title, image, time, link }: HHCardProps) => {
  return (
    <a href={link} className="movie-card group cursor-pointer block">
      <div className="relative aspect-video overflow-hidden rounded-t-xl">
        ...
        <div className="absolute bottom-3 right-3">
          <span className="px-2.5 py-1 bg-background/80 backdrop-blur-sm text-foreground text-[10px] font-mono font-bold rounded-md border border-border/50">
            {time}
          </span>
        </div>
      </div>
